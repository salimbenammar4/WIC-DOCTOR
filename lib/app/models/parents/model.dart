...
abstract class Model {
  ...
  String transStringFromJson(
    ...
    try {
      if (json == null || json[attribute] == null) return defaultValue;

      // If the attribute is a map (e.g., for translations)
      if (json[attribute] is Map<String, dynamic>) {
        final translations = json[attribute] as Map<String, dynamic>;
        // Use the default locale, fallback to the current locale, or return default value
        final locale = defaultLocale ?? Get.locale?.languageCode ?? 'fr';
        final translatedValue = translations[locale] ?? translations['fr'];

        // Return the translated value if found and valid, else return the default value
        if (translatedValue != null && translatedValue != 'null') {
          return translatedValue.toString();
        }
        return defaultValue;
      }

      // If the attribute is a direct value
      return json[attribute].toString();
    } catch (e) {
      throw Exception('Error parsing $attribute: ${e.toString()}');
    }
  }
